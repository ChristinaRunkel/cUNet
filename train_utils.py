...
def evaluate_image(net, image, test_time_augmentation=True, shouldpad=False):
    ...
    with torch.no_grad():
        if shouldpad:
            pad = 192
            padded_np_image = np.pad(
                image,
                ((pad // 2, pad // 2), (pad // 2, pad // 2), (0, 0)),
                mode="reflect",
            )
        ...
        if test_time_augmentation:
            ...
            soft_result_np = (
                soft_result_np + soft_result_np_ud + soft_result_np_lr
            ) / 3
...
def pad_image(image, size=(352, 512)):
    ...
    # pad to image size
    padded_image = np.pad(
        image, ((pad_h[0], pad_h[1]), (pad_w[0], pad_w[1]), (0, 0)), mode="reflect"
    )
    return padded_image
...
def crop_result(result, image):
    ...
    result = result[
        :, pad_h[0] : result.shape[1] - pad_h[1], pad_w[0] : result.shape[2] - pad_w[1]
    ]
    return result
...
def save_loss_to_file(logfile, eval_type, loss):
    """
    Appends a formatted loss value and evaluation type to a log file with a timestamp.

    Args:
        logfile (str): Path to the log file.
        eval_type (str): Type of evaluation (e.g., 'train', 'val').
