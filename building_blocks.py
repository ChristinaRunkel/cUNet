...
class InitialVelocity(nn.Module):
    def __init__(self, nf, non_linearity="relu"):
        """
        Initializes the InitialVelocity block for 2D data.
        Args:
            nf (int): Number of filters.
            non_linearity (str): Non-linearity to use.
        """
    ...
    def forward(self, x0):
        """
        Forward pass for InitialVelocity block.
        Args:
            x0 (Tensor): Input tensor.
        Returns:
            Tensor: Concatenated input and output features.
        """
        out = self.norm1(x0)
        out = self.conv1(out)
...
class InitialVelocity3D(nn.Module):
    def __init__(self, nf, non_linearity="relu"):
        """
        Initializes the InitialVelocity block for 3D data.
        Args:
            nf (int): Number of filters.
            non_linearity (str): Non-linearity to use.
        """
    ...
    def forward(self, x0):
        """
        Forward pass for InitialVelocity3D block.
        Args:
            x0 (Tensor): Input tensor.
        Returns:
            Tensor: Concatenated input and output features.
...
class ConvResFunc(nn.Module):
    ...
    def forward(self, x):
        """
        Forward pass for ConvResFunc block.
        Args:
...
class ConvDenseBlock(nn.Module):
    def __init__(
        self, in_channels, out_channels, bn_size=4, drop_rate=0, memory_efficient=False
    ):
        """
        ...
        Args:
            ...
            memory_efficient (bool): Use checkpointing for memory efficiency.
        """
        ...
        self.add_module(
            "conv1",
            nn.Conv2d(
                in_channels, bn_size * out_channels, kernel_size=1, stride=1, bias=False
            ),
        ),
    ...
    def bn_function(self, inputs):
        """
        Bottleneck function for dense block.
        Args:
            inputs (list of Tensor): List of input feature tensors.
        Returns:
            Tensor: Output after bottleneck transformation.
        """
    ...
    def forward(self, input):  # noqa: F811
        """
        Forward pass for ConvDenseBlock.
        Args:
            input (Tensor or list of Tensor): Input tensor(s).
        Returns:
            Tensor: Output tensor after dense block.
        """
...
class INCConvBlock(nn.Module):
    def __init__(self, in_channels, out_chanels, **kwargs):
        """
        Inception convolution block with batch normalization and ReLU.
        Args:
            in_channels (int): Number of input channels.
...
class PSPBlock(nn.Module):
    def __init__(self, in_channels, out_channels, sizes=(1, 2, 3, 6)):
        ...
        self.bottleneck = nn.Conv2d(
            in_channels * (len(sizes) + 1), out_channels, kernel_size=1
        )
        self.relu = nn.ReLU()
