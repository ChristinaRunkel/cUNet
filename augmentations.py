...
def ElasticTransformations(
    alpha=2000, sigma=40, random_state=np.random.RandomState(42)
):
    """
    ...
    def distort_elastic_cv2(image):
        """
        Applies elastic distortion to a single image using random displacement fields.
        Args:
            image (PIL.Image or np.ndarray): Input image to be distorted.
        Returns:
...
def RandomRotationWithMask(degrees, resample=False, expand=True, center=None):
    ...
    def _rotate(img):
        """
        Rotates a 4D image (with mask in last channel) by a random angle.
        Args:
            img (PIL.Image or np.ndarray): Input image with mask as last channel.
        Returns:
            PIL.Image: Rotated image with mask.
        """
