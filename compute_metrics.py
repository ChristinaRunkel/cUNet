...
from metrics import averaged_hausdorff_distance, pixelwise_acc  # compute_dice
...
parser.add_argument("--solver", type=str, nargs="*", help="Solver")


def load_dataset(dataset: str) -> Tuple[torch.utils.data.Dataset, float]:
    """
    Loads the specified dataset and returns the validation set and image size.

    Args:
        dataset (str): Name of the dataset to load.

    Returns:
        Tuple[torch.utils.data.Dataset, float]: Validation set and image size.
    """
    if dataset == "cells":
...
def compute_metrics(
    ...
    """
    Computes segmentation metrics for a given model and dataset.
    ...
    if net_name == "USODE":
        ...
        net = Unet(depth=4, num_filters=3, output_dim=output_dim, block="PLN")
        ...
        config.hidden_size = 768  # 384 # adapt (value for img_size=512: 768)
        config.transformer = ml_collections.ConfigDict()
        config.transformer.mlp_dim = 3072  # 1536 #adapt (value for img_size=512: 3072)
        ...
        config.patch_size = 32  # 16  # adapt (value for img_size=512: 32)
        ...
        config.patches.grid = (
            32,
            32,
        )  # (16, 16)  # adapt (values for img_size=512: (32, 32))
    ...
    for img, lbl in testloader:
        ...
        # dice += compute_dice(y_pred=out_thres, y=lbl_detach).item()
    ...
    print(
        f"Solver: {solver} - Dice: {dice:.4f} - Avg Hausdorff: {avg_hausdorff:.2f} - Accuracy: {acc:.4f}"
    )
...
def write_to_log_file(solver, net_name, log_file, metric_name, metric_value):
    """
    Writes a metric value to a log file with a formatted line.
    ...
    if args.solver is not None:
        for identifier, net_name, solver in zip(args.identifier, args.net, args.solver):
            ...
            log_file = os.path.join(
                args.path, args.dataset, "log_files", "METRICS_SOLVERS.txt"
            )
        ...
        for identifier, net_name in zip(args.identifier, args.net):
            print(identifier, net_name)
            log_file = os.path.join(args.path, args.dataset, "log_files", "METRICS.txt")
            dice, avg_hausdorff, acc = compute_metrics(
                net_name=net_name,
                dataset=args.dataset,
                valset=valset,
