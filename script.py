...
from transforms import RandomRotation, RandomVerticalFlip, RandomHorizontalFlip
from train_utils import save_loss_to_file


parser = argparse.ArgumentParser(description="USODE experiments")
parser.add_argument(
    "--epochs",
    default=500,
    type=int,
...
def main():
    """
    Main entry point for training and evaluation script.
    Parses arguments, sets seeds, loads data, and initializes model and device.
    """
    ...
    if args.net == "USODE":
        ...
        output_dim_for_unet = 2 if args.dataset == "cells" else 1
        net = Unet(
            depth=4, num_filters=3, output_dim=output_dim_for_unet, block=args.block
        )
        ...
        config.hidden_size = 768  # adapt (value for img_size=512: 768)
        config.transformer = ml_collections.ConfigDict()
        config.transformer.mlp_dim = 3072  # adapt (value for img_size=512: 3072)
        ...
        config.patch_size = 32  # adapt (value for img_size=512: 32)

        config.decoder_channels = (256, 128, 64, 32)
        config.n_classes = 1
        config.activation = "softmax"
        config.patches.grid = (32, 32)  # adapt (values for img_size=512: (32, 32))
    ...
    elif args.net == "UNETPLUSPLUS":
        ########## New Model from Lihao
        from other_models import BasicUNetPlusPlus

        net = BasicUNetPlusPlus(spatial_dims=2, in_channels=3, out_channels=1)
    elif args.net == "U2NET":
        ########## New Model from Lihao
        from other_models import U2NET
        ...
        raise ValueError(
            "Please pick one of the following network architectures: USODE, UNODE, UNET, URESNET, UNEXT, DENSEUNET, REGUNET, ATTENTIONUNET, DYNUNET, UNETPLUSPLUS, U2NET, DEEPLABV3PLUS"
        )
    if args.pretrained:
        if args.net == "UNET":
            net = torch.hub.load(
    ...
    filename = os.path.join(
        ...
        args.dataset,
        "checkpoints",
        args.net + "-" + args.solver + "-" + date_str + ".pt",
    )
    filename_best = os.path.join(
        "<PATH_TO_OUTPUTS>",
        args.dataset,
        "checkpoints",
        args.net + "-" + args.solver + "-" + date_str + "_best.pt",
    )
    ...
    logfile = os.path.join(
        ...
        args.dataset,
        "log_files",
        args.net + "-" + args.solver + "-" + date_str + ".txt",
    )
    run(
        args=args,
        logfile=logfile,
...
def run(
    ...
    """
    Runs the training and validation loop for the model.
    Tracks losses, saves checkpoints, and logs training progress.
    ...
    Args:
        ...
        filename (str): Path to save model checkpoints.
        filename_best (str): Path to save best model checkpoint.
    """
    losses = []
    val_losses = []
    accumulated = 0
