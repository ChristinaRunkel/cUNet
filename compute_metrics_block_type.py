...
def compute_metrics(
    ...
    """
    Computes segmentation metrics for a given model, block type, and dataset.

    Args:
        net_name (str): Name of the network/model.
        block_type (str): Type of block used in the model.
    ...
    if net_name == "USODE":
        ...
        net = Unet(depth=4, num_filters=3, output_dim=output_dim, block=block_type)
...
def write_to_log_file(log_file, metric_name, metric_value):
    """
...
if __name__ == "__main__":
    ...
    for identifier, net_name, block_type in zip(args.identifier, args.net, args.block):
        print(identifier, net_name, block_type)
        log_file = os.path.join(
            args.path, args.dataset, "log_files", identifier + ".txt"
        )
        dice, hausdorff, f1 = compute_metrics(
            net_name=net_name,
        ...
        # write_to_log_file(log_file=log_file, metric_name="Dice", metric_value=dice)
        # write_to_log_file(log_file=log_file, metric_name="Hausdorff", metric_value=hausdorff)
        # write_to_log_file(log_file=log_file, metric_name="F1", metric_value=f1)
